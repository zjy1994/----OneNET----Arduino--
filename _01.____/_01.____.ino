...
HttpPacketHead packet; //实例化一个HttpPacketHead对象
...
const unsigned int esp8266RxBufferLength = 600;     // 新建一个字符型数组，含600个元素
...
void ESP8266_ERROR(int num)
{
  ...
  while (1)
  {
    ...
    if (sendCommand("AT\r\n", "OK", 100, 10) == Success)  // 如果"AT"这条命令发送成功了，就执行if里面的语句
    {
      DebugSerial.print("\r\nRESET!!!!!!\r\n");
      resetFunc();
    }
...
/*
 * 功能：将 通过串口（ESP8266Serail）读到的数据 存放到esp8266RxBuffer这个字符型数组
 */
void esp8266ReadBuffer() {
  while (ESP8266Serail.available())
  {
    esp8266RxBuffer[ii++] = ESP8266Serail.read();   // 一个字节一个字节地读取
